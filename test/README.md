...
- guards duplicate `.gitmodules` path mappings across different submodule owners (fixture-level parser behavior + live-repo check), while allowing repeated identical owner/path declarations and deduplicating repeated duplicate-owner records.
...
- verifies unsafe gitlink path inputs (traversal/absolute/scheme/control-char) are surfaced as explicit validator errors.
